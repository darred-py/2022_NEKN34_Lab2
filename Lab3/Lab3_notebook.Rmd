...
astsa::tsplot(income.ts, spaghetti = T)
...
```{r}
#Checking for stationarity at the first difference
tseries::adf.test(diff(income.ts[,1]))
tseries::adf.test(diff(income.ts[,2]))
tseries::adf.test(diff(income.ts[,3]))
# tseries::adf.test(diff(income.ts[,4]))
# tseries::adf.test(diff(income.ts[,5]))

# Results: ALl time series are stationary at first differences
```
...
plot(income.data$m1, income.data$ip, type = "l")
plot(income.data$m1, income.data$curr_circ, type = "l")
...
plot(income.data$ip, income.data$curr_circ, type = "l")
```
...
```{r}
formula1 <- "ip ~ m1 + curr_circ" #We have a lower p-value when using sqrt(M1) vs log(M1)
